...
import dev.backend.epharmaApp.dao.entity.UserRolesKey;
import dev.backend.epharmaApp.dao.repository.customize._UserRolesRepository;

/**
 * Repository : UserRoles.
 */
@Repository
public interface UserRolesRepository extends JpaRepository<UserRoles, UserRolesKey>, _UserRolesRepository {
	/**
	 ...
	 */
	@Query("select e from UserRoles e where e.users.userId = :userId and e.isDeleted = :isDeleted")
	UserRoles findOneByUserId(@Param("userId")Integer userId, @Param("isDeleted")Boolean isDeleted);
	 ...
	 *         no UserRoles is found, this method returns null.
	 */
	@Query("select e from UserRoles e where e.roles.roleId = :roleId and e.isDeleted = :isDeleted")
	UserRoles findOneByRoleId(@Param("roleId")Integer roleId, @Param("isDeleted")Boolean isDeleted);

	/**
	 * Finds UserRoles by using recordStatus as a search criteria.
	 * 
	 * @param recordStatus
	 * @return An Object UserRoles whose recordStatus is equals to the given recordStatus. If
	 *         no UserRoles is found, this method returns null.
	 */
	@Query("select e from UserRoles e where e.recordStatus = :recordStatus and e.isDeleted = :isDeleted")
	List<UserRoles> findByRecordStatus(@Param("recordStatus")String recordStatus, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	default List<UserRoles> getByCriteria(Request<UserRolesDto> request, EntityManager em, Locale locale) throws DataAccessException, Exception {
		...
		req += " order by e.userIde.roleId desc";
	...
	default Long count(Request<UserRolesDto> request, EntityManager em, Locale locale) throws DataAccessException, Exception  {
		...
		HashMap<String, java.lang.Object> param = new HashMap<String, java.lang.Object>();
		req += getWhereExpression(request, param, locale);
		req += " order by  e.userId e.roleId desc";
