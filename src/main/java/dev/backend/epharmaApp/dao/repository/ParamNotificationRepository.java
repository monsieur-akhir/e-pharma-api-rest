...
import dev.backend.epharmaApp.dao.repository.customize._ParamNotificationRepository;

/**
 * Repository : ParamNotification.
 */
@Repository
public interface ParamNotificationRepository extends JpaRepository<ParamNotification, Integer>, _ParamNotificationRepository {
	/**
	...
	ParamNotification findOne(@Param("id")Integer id, @Param("isDeleted")Boolean isDeleted);

	@Query("select e from ParamNotification e where e.type = :type and e.lang = :lang and e.isDeleted = :isDeleted")
	ParamNotification findByLangAndNotifType(@Param("type")String type, @Param("lang")String lang, @Param("isDeleted")Boolean isDeleted);
	...
	List<ParamNotification> findByCreatedAt(@Param("createdAt")Date createdAt, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	List<ParamNotification> findByCreatedBy(@Param("createdBy")Integer createdBy, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	List<ParamNotification> findByIsDeleted(@Param("isDeleted")Boolean isDeleted);
	/**
	...
	List<ParamNotification> findByLang(@Param("lang")String lang, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	List<ParamNotification> findByMessage(@Param("message")String message, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	List<ParamNotification> findByType(@Param("type")String type, @Param("isDeleted")Boolean isDeleted);

	@Query("select e from ParamNotification e where e.type = :type and e.lang = :lang and e.isDeleted = :isDeleted")
	ParamNotification findByNotificationType(@Param("type")String type, @Param("lang")String lang, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	List<ParamNotification> findByUpdatedAt(@Param("updatedAt")Date updatedAt, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	List<ParamNotification> findByUpdatedBy(@Param("updatedBy")Integer updatedBy, @Param("isDeleted")Boolean isDeleted);
	 ...
	 */
	default List<ParamNotification> getByCriteria(Request<ParamNotificationDto> request, EntityManager em, Locale locale) throws DataAccessException, Exception {
		String req = "select e from ParamNotification e where e IS NOT NULL";
		HashMap<String, Object> param = new HashMap<String, Object>();
		req += getWhereExpression(request, param, locale);
		req += " order by e.id desc";
		TypedQuery<ParamNotification> query = em.createQuery(req, ParamNotification.class);
	 ...
	 * 
	 */
	default Long count(Request<ParamNotificationDto> request, EntityManager em, Locale locale) throws DataAccessException, Exception  {
	 ...
	 * @throws Exception
	 */
	default String getWhereExpression(Request<ParamNotificationDto> request, HashMap<String, Object> param, Locale locale) throws Exception {
		// main query
		ParamNotificationDto dto = request.getData() != null ? request.getData() : new ParamNotificationDto();
		...
		if (request.getDatas() != null && !request.getDatas().isEmpty()) {
			Integer index = 1;
			for (ParamNotificationDto elt : request.getDatas()) {
	...
	default String generateCriteria(ParamNotificationDto dto, HashMap<String, Object> param, Integer index,  Locale locale) throws Exception{
		...
		if (dto != null) {
			...
			if (Utilities.notBlank(dto.getLang())) {
				listOfQuery.add(CriteriaUtils.generateCriteria("lang", dto.getLang(), "e.lang", "String", dto.getLangParam(), param, index, locale));
			}
