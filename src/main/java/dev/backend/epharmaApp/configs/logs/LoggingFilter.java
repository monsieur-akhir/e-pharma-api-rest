package dev.backend.epharmaApp.configs.logs;

import com.google.gson.*;
import com.google.gson.stream.JsonReader;
import dev.backend.epharmaApp.dao.entity.Logs;
import dev.backend.epharmaApp.dao.entity.Users;
import dev.backend.epharmaApp.dao.repository.LogsRepository;
import dev.backend.epharmaApp.dao.repository.UsersRepository;
import dev.backend.epharmaApp.utils.CustomEntityNotFoundException;
import dev.backend.epharmaApp.utils.ParamsUtils;
...
public class LoggingFilter extends OncePerRequestFilter {
	...
	private Logs createLogsDto(HttpServletRequest request, String requestBody, String responseBody, String ipAddress,
	    ...
	    logs.setCreatedBy(userId != null ? userId : (user != null ? user.getUserId() : null));
	...
	@SneakyThrows
	private Users getUser(String requestBody) {
		...
		try {
			...
			Users user = userRepository.findOne(userId, Boolean.FALSE);
			if (user == null) {
	...
	private Users getUserById(Integer userId) {
		...
		Users user = userRepository.findOne(userId, false);
		if (user == null) {
	...
	private Users getUserByLogin(String login) {
		...
		Users user = userRepository.findByLogin(login, false);
		if (user == null) {
