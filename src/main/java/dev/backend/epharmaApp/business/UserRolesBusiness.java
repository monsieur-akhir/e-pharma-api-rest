...
//			fieldsToVerify.put("deletedBy", dto.getDeletedBy());
			...
			if (existingEntity != null) {
				response.setStatus(functionalError.DATA_EXIST("userRoles userId -> " + dto.getUserId(), locale));
				response.setHasError(true);
				return response;
			}

			if (existingEntity != null) {
				response.setStatus(functionalError.DATA_EXIST("userRoles roleId -> " + dto.getRoleId(), locale));
				response.setHasError(true);
				return response;
			}
	...
	public Response<UserRolesDto> update(Request<UserRolesDto> request, Locale locale)  throws ParseException {
		...
		for (UserRolesDto dto : request.getDatas()) {
			...
			UserRoles entityToSave = null;
			entityToSave = userRolesRepository.findOneByUserId(dto.getUserId(), false);
			if (entityToSave == null) {
				response.setStatus(functionalError.DATA_NOT_EXIST("userRoles userId -> " + dto.getUserId(), locale));
				response.setHasError(true);
				return response;
			}
			entityToSave = userRolesRepository.findOneByRoleId(dto.getRoleId(), false);
			if (entityToSave == null) {
				response.setStatus(functionalError.DATA_NOT_EXIST("userRoles roleId -> " + dto.getRoleId(), locale));
				response.setHasError(true);
				return response;
			}
	...
	public Response<UserRolesDto> delete(Request<UserRolesDto> request, Locale locale)  {
		...
		for (UserRolesDto dto : request.getDatas()) {
			...
			// Verifier si la userRoles existe
			UserRoles existingEntity = null;
			existingEntity = userRolesRepository.findOneByUserId(dto.getUserId(), false);
			if (existingEntity == null) {
				response.setStatus(functionalError.DATA_NOT_EXIST("userRoles -> " + dto.getUserId(), locale));
				response.setHasError(true);
				return response;
			}
			existingEntity = userRolesRepository.findOneByRoleId(dto.getRoleId(), false);
			if (existingEntity == null) {
				response.setStatus(functionalError.DATA_NOT_EXIST("userRoles -> " + dto.getRoleId(), locale));
				response.setHasError(true);
				return response;
			}
